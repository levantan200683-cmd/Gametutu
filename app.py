...
def gui_du_lieu_server(ten, diem, so_tran, mat_khau_moi="", cheat_mode=False):
    try:
...
st.caption("Bản Web-App đăng nhập bảo mật - Giao diện tối ưu")
...
if st.session_state.game_state == 'MENU':
    ...
    if ten:
        ...
        # Giao diện khi Tên ĐÃ TỒN TẠI (Đăng nhập)
        if user_he_thong:
            ...
            with col_u1:
                if st.button("🚀 ĐĂNG NHẬP & CHƠI", use_container_width=True):
                    ...
                    # Kiểm tra mật khẩu (hỗ trợ các acc cũ chưa có mật khẩu mặc định là rỗng hoặc 123)
        ...
        # Giao diện khi Tên CHƯA TỒN TẠI (Đăng ký mới)
        else:
    ...
    if admin_code == "lapdepzai":
        if ten:
            # Phải nhập mật khẩu đúng của nick đó trước (nếu nick đã tồn tại) mới cho dùng quyền Admin
            if user_he_thong:
                mk_check = st.text_input("🔑 Xác minh mật khẩu Admin của nick hiện tại:", type="password", key="ad_pass")
                if st.button("Xác nhận quyền Admin"):
                    if ma_hoa_mat_khau(mk_check) == user_he_thong.get("matkhau", ""):
                        st.session_state.game_state = 'ADMIN'
                        st.rerun()
                    else: st.error("Mật khẩu nhân vật không đúng, không thể dùng lệnh Admin!")
            else:
                st.session_state.game_state = 'ADMIN'
                st.rerun()
        else:
            st.warning("Hãy nhập một tên nhân vật bất kỳ trước khi kích hoạt Admin!")
...
# --- MÀN HÌNH ADMIN BÍ MẬT ---
    ...
    sotran_sua = st.number_input("👉 Cài đặt SỐ TRẬN mới:", min_value=1, value=50)
